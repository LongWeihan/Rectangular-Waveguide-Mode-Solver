...
if ~isempty(single_mode_indices)
    ...
    fprintf('Waveguide size at the center of single-mode region: a = %.3f µm\n', a_single_mid);
    fprintf('Corresponding effective index: N_eff = %.4f\n', Neff_single_mid);
else
    fprintf('No single-mode region found, cannot compute effective index at center.\n');
end
...
xlabel('Waveguide side length a (\mum)', 'FontSize', 12);
ylabel('Effective index N_{eff}', 'FontSize', 12);
title('Dispersion curves of effective indices for different modes of a rectangular waveguide', 'FontSize', 10, 'FontWeight', 'bold');
