...
n_sub  = 1.4442;
n_clad = 1.512;
a_vec = linspace(0.2e-6, 10.0e-6, 200);
maxVer = 5;
maxLat = 5;
modeCurves = {};
...
if ~isempty(single_mode_indices)
    ...
    fprintf('Single-mode region: a ∈ [%.3f, %.3f] µm\n', a_single_min, a_single_max);
else
...
if ~isempty(dual_mode_indices)
    ...
    fprintf('Dual-mode region: a ∈ [%.3f, %.3f] µm\n', a_dual_min, a_dual_max);
else
...
if ~isempty(single_mode_indices)
    ...
    fprintf('Waveguide width at center of single-mode region: a = %.3f µm\n', a_single_mid);
    fprintf('Corresponding effective refractive index: N_eff = %.4f\n', Neff_single_mid);
else
    fprintf('No single-mode region found; cannot compute effective index at midpoint.\n');
end
...
xlabel('Waveguide width a (\mum)', 'FontSize', 12);
ylabel('Effective refractive index N_{eff}', 'FontSize', 12);
title('Dispersion curves of effective refractive index vs waveguide size for various modes', 'FontSize', 10, 'FontWeight', 'bold');
